...
until [[ $(kubectl get ingress server-ingress -o jsonpath='{.status.loadBalancer.ingress[0].ip}') ]] || [[ $(kubectl get ingress server-ingress -o jsonpath='{.status.loadBalancer.ingress[0].hostname}') ]]
do
